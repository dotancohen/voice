...
| ACCT-6 | **Several accounts on one installation** (Stage 2). The root (`$VOICE_CONFIG_DIR`, else `~/.config/voice`) holds the machine's `config.json`, `accounts.db` and one directory per account, named by the account id. `resolve(root, selector)` opens the account named by `-a` (id, unique id prefix, or label) or `$VOICE_ACCOUNT_ID`, else the default; with no index and no default on a first run, it makes one. A root that already holds `notes.db` or `config.json` and no index is the account itself (the phone, a test directory), and `-a` is refused there. |
| ACCT-7 | `account list`, `account create [--label]`, `account default <selector>`, `account remove <selector>` (the index row only). Labels are unique. The first account that is not hosted becomes the default; a hosted account never is. |
| ACCT-8 | Machine settings (the device id and name, the listen port, the backup, the public URL) live in the root's `config.json`, override what an account's file says, and are written back there; the certificates live under the root. Everything else, the device key included, is the account's. Each account's audio directory defaults to `audio/` inside the account. |
| ACCT-9 | The index is only an index: opening an account checks that its database carries the account's id (ACCT-4), and a disagreement is an error, never corrected. |
| ACCT-10 | **Moving a device to another account** (`account move --to <setup text> --current <full id typed by hand>`; the phone's Advanced Settings): a snapshot, the code claimed for a key of the other account, the account id rewritten and every peer forgotten, the notes left in place, the other account's tags pulled and tags with one path merged (the older id kept, links and children moved through the versioned entities, the duplicate deleted), then everything exchanged. No GUI or TUI path on the desktop; a join still refuses a device with notes. |
...
| AUTH-4 | The two headers name the account and the device, so a server that hosts several accounts routes and authenticates in one step. A listener over one directory serves that one account; a listener over an indexed root (`sync serve` there) serves every account of the index, opening each on its first request and keeping at most 64 open. An account the server does not hold is `ACCOUNT_UNKNOWN` (404), counted against the caller's address like a bad key. |
...
| CARD-3 | **The cards are the peer list.** After every sync, a card of the account without a peer entry gets one (the card's name, its first address, its fingerprint); an entry re-pins its fingerprint from the card, which arrived over an authenticated connection, so nobody types a fingerprint twice; a revoked card's entry goes. The remembered address (`sync_peers`, the config's `peer_url`) is never replaced by the card's; a local name (`rename-peer`) stays over the card's; a forgotten peer (`remove-peer`, the config's `forgotten_peers`) is not brought back by its card until it is added again by hand or by pairing. A peer typed by hand (id, name, address) is the third way to find a device. |
| LISTEN-3 | **A listener serves the LAN, not the internet**, unless the machine has a `public_url` configured: a request from an address that is not private (RFC 1918), link-local or this machine itself is refused before any route with `NOT_ON_LAN`. A phone on hotel wifi is thereby not a server for the hotel. |
| DISC-1 | **mDNS**: while it listens, a device announces `_voicesync._tcp` (desktop: `zeroconf`; phone: `NsdManager`) with a TXT record of `v`, `a` = the SHA-256 of the account id (never the id), `d` = its device id, `n` = its name, `f` = its certificate fingerprint. `sync serve --plain-http` and `--no-announce` announce nothing. |
| DISC-2 | **Three ways to find a peer, remembered address first**: an operation runs with the remembered address; only when the peer is not reached there (silence, not a refusal) does the caller browse for the device id under the account's hash, remember the address that answered (and its fingerprint), and run once more. A peer can also be typed by hand from its id and address. `sync discover` and "Find on this network" list the devices of the account announcing nearby, and a found one can be added as a peer. |
| DISC-3 | Every route still checks the account and the key, so a right-looking address of the wrong account is refused, never merged. |
| BUCKET-1 | **The wizard** (File → Set up the bucket…, `storage setup`): the console steps one at a time with the policy text and a copy button; the pasted key cleaned of whitespace and labels; a region, the nearest by round-trip time proposed; a generated `voice-…` name, changeable, a taken one answered with another; the bucket made private; hardened (BUCKET-3); the lifecycle rules (BUCKET-4); a round trip (a small object written, read back, compared, tagged purged); saved through the synced storage configuration; the listener offered; "Test everything" last. Every failure is a sentence (`bucket_setup::explain_error`). The values are kept for the session if the window closes; the secret is written nowhere until the save. |
| BUCKET-2 | **The key cannot delete.** The policy grants create, list, the bucket's own settings, put, get, tagging and multipart on `voice-*`, and nothing else. A purge tags the object `voice-purged=1` (remembered locally in `purged_objects`, tagged at the next upload run) and the lifecycle rule deletes it a day later. "Replace key" (File → Replace the bucket's key…, `storage replace-key`) tests a new key with the round trip and saves it for every device. |
| BUCKET-3 | **Hardened on creation**, three signed requests and three reads back: public access blocked, default encryption (AES256), a bucket policy that refuses anything not over TLS. `storage check` and "Test everything" read them back later. |
| BUCKET-4 | **Lifecycle**: objects move to the infrequent-access class after 30 days; objects tagged purged expire after 1 day; incomplete multipart uploads are abandoned after 2 days. Versioning is never turned on. |
| BUCKET-5 | **Upload asks first**: a pending row whose object is already in the bucket (after a snapshot restore, say) gets its `storage_key` from one request instead of a whole file. |
| PROTO-12 | **Protocol version 2.** Both sides announce `2.0`; a handshake from a `1.x` peer is refused with HTTP 426 and `PROTOCOL_TOO_OLD`, "Update Voice on <device name>", and a caller refuses a `1.x` responder the same way. Nothing negotiates with version 1; everything started afresh. |
| PROTO-13 | **Room for other applications.** The handshake carries `application` and `entity_types`; the changes feed takes `?types=tag,...` and returns only those (the cursor still walks the whole feed); apply accepts only the types the peer declared, and says how many it did not apply. An image application that declares `["tag"]` receives and sends tags and nothing else. The device card carries `application`. |
| FILE-17 | **Cancel and progress.** Every operation reports progress (the stage, how many of how many, the bytes moved, a sentence) to a sink the interface gives it: the phone's foreground-service notification, the desktop dialogue's label. A cancel (the notification's Cancel action, the dialogue's button, `SyncClient::cancel`) stops the operation at its next page, file or chunk; a transfer under way stays a part and the next operation continues from it; the result says "Cancelled". A cancel is never retried. |
| LISTEN-4 | **The listener stops itself** after `sync.listener_idle_stop_hours` of silence (never, one, four or eight hours, chosen on the sync screen and in the desktop dialogue; never is the default), measured from its last request or its start. Stopping, not starting: nothing here starts a listener. |
| FILE-15 | **A recording's file name is on its row** (`audio_files.disk_name`) and is always the name the file has on disk, on every device: the name is synced, and a device holding the file renames it when the name changes. A file this application records is named `YYYY_MM_DD_HH_MM_SS-<last eight of the id>.<ext>`, the start rendered at the recording's own offset. An imported file keeps its own name, any POSIX name, in the same folder. **A collision** is two recordings that are not deleted with identical names, wherever they meet: an import, a sync, a fetch or a download. Both get `-<last eight characters of their own id>` before the extension, or `-<the whole id>` when that is still taken, so every device computes the same names; a later file whose name already collided gets its suffix too. Names are identical when their bytes are, and on the phone's storage, which does not tell upper from lower case, also when they differ only in case. A deleted recording's name is set in stone: its row and its file are never renamed, and a new file whose name it holds, or whose name deleted recordings took in an old collision, takes its own suffix; a file on disk that no row names moves aside. The copy into the folder never overwrites a file. A rename waits for a caller that knows the folder before it reaches the disk: after a sync, at start, and before a file is read or written. A sync that renames recordings while pulling pushes those rows in the same sync, so one exchange leaves both devices with the same names. A row from before the column keeps the name its file has, `<id>.<ext>`: no migration changes a name that refers to a file. |
| FILE-16 | On the phone the default audio directory is the shared `Recordings/Voice` folder once all-files access is granted (TECHNICAL-DECISIONS 3.1a); it holds recordings and nothing else. |
| FILE-18 | **A recording's content hash** (`audio_files.content_sha256`, SHA-256 of the bytes, lowercase hex) is synced metadata merged per column: written after the file is copied at import and at recording (`store_content_hash` on every platform), taken from a newer row that has one, never erased by a row without one. The bucket object is keyed by it (`<hash>.<ext>`), so two devices importing one file make one object; a bucket download whose bytes do not hash to it is removed and reported; a send uses the stored hash instead of hashing the file again, and stores it once when the row has none. |
| FILE-19 | **A file larger than one part goes to the bucket in parts** (8 MiB each; Amazon's minimum is 5 MiB) through the bucket's three requests, signed by the core itself, never through a library call that aborts on failure. The upload id, the part size and each part's tag are written to `upload_parts` as each part lands; the next upload run continues the remembered upload when its key and part size still fit and sends only the parts missing; the object exists only once every part is there; a cancel stops before the next part. Abandoned uploads are the bucket's lifecycle rule's to remove (BUCKET-3), never a delete by the key. |
| FILE-20 | **The levels a waveform is drawn from travel with the recording** (`audio_files.waveform_levels`: 1 to 4,096 values from 0 to 255, the loudest share at 255, as base64url text). Whichever device decodes a recording first keeps them: the desktop when it draws a waveform, when the text interface shows one, and for every recording on it that has none through `voice cli audiofiles-waveforms`; the phone when it draws a waveform. Every device draws the bars from them (`waveform::bars_from_levels`: the loudest level of each share, scaled to the loudest bar) before it reads or decodes the file, so the phone never decodes a recording a desktop decoded, and a recording with levels is never offered as "large" first. A newer row's levels replace an older's; a row without levels, or with text that is not levels, never erases them. |
| FILE-21 | **Every common audio format is imported** (the owner's decision, 2026-09-13), by extension from one list in the core (`models::AUDIO_FILE_FORMATS`) that both bindings export: 3g2, 3ga, 3gp, aac, ac3, aif, aifc, aiff, amr, ape, au, awb, caf, flac, gsm, m4a, m4b, mka, mp2, mp3, mp4, mpga, oga, ogg, opus, qcp, snd, spx, wav, wave, weba, webm, wma, wv. A file with any other extension is skipped. The desktop plays and decodes through mpv and FFmpeg. The phone's player uses the phone's own decoders first and FFmpeg's decoders (Jellyfin's build for media3) for the rest; its containers are those media3 reads, so WMA, AIFF, CAF, APE, WavPack, AU, GSM, QCP and Speex files do not play on the phone. |
| FILE-22 | **Where each copy of a recording is, synced** (`file_locations`): one row per recording and place, a place being a device of the account (by id) or `cloud`, with whether it holds the file, when that was stated (milliseconds) and by which device. The newest statement about a place wins, then the larger device id, then presence, so every device keeps the same rows whatever order they arrive in; different places never conflict, so a device removing its copy while another uploads the file are both kept. A device states its own copy when it receives a file whole, fetches or downloads one, and when a sync (or the Issues list, or "Where are the copies?") compares its audio folder with what it has stated: a file that is there is stated here, a file that is gone is stated gone, and a folder that is not there at all states nothing. The bucket's row is stated by an upload, by clearing an upload, and by a download that finds the object gone. A sender or a fetcher states that the peer holds the file; the peer's own statement replaces that one when newer. "Remove from this device" deletes this device's file and states it gone, and is refused when no other place holds the file. The rows go with a purged recording. An older database starts from what its rows said: the bucket for uploaded rows, and peers from `audio_file_copies`. |
| FILE-23 | **The upload limit is the account's** (`max_upload_mb` in the synced storage configuration, 100 MB until set, at least 1 MB): an upload leaves a larger file where it is and counts it (`too_large`), on every device alike. The size of each file (`audio_files.size_bytes`) is synced, written with the content hash, and never changed once known. The per-device `sync.max_sync_file_size_mb` is the listener's body limit for its JSON routes and nothing else. |
| ISSUE-1 | **Issues: what the user should know about**, computed at every look and never stored: recordings not in the bucket, each with the reason that is true of it (no bucket, over the account's upload limit, waiting for the named devices that hold it, or no copy known); transcriptions whose recording row is not there; attachments whose note or recording row is not there; recordings no note holds (a note in the trash still holds its recordings); tags whose names contain whitespace, with their paths. The folder is compared first. Opened when asked for (File → Issues…, F8 in the text interface, `cli issues`, `GET /api/issues`, Settings → Issues on the phone), never pushed at the user. |
| ENC-1 | **One recording key per account** (Stage 15): 32 random bytes, 43 base64url characters, kept where the device key is kept (wrapped on the phone, AUTH-9). It travels to a device let in by the code in the claim reply and to a hosting server in the grant, over TLS. Export (`account recording-key export`, "Export the recording key" on the desktop and under Advanced Settings on the phone) shows it as text and a QR code labelled "Recording key. Keep this on paper. Without it these recordings cannot be played."; import keeps one from an export. The encryption switch stays off until the key was exported from that device. |
| ENC-2 | **Files are encrypted in chunks** of one MiB with AES-256-GCM: a header `VOICEENC`, version 1, a random 12-byte file nonce; chunk `i` under a nonce whose last four bytes are the file nonce's XOR `i` (the plan's wording, one nonce per file with the index only in the associated data, would repeat a nonce under one key across chunks, which GCM does not survive); the header, the index and whether the chunk is the last are the associated data, so a changed byte, a swapped chunk or a cut-off end refuses to open. Memory is one chunk. |
| ENC-3 | **Encryption is a synced setting** (`encrypt` in the storage configuration), off by default. On, every upload goes in parts through the cipher without a second copy on disk, the object is `<hash>.<ext>.enc`, and the row says `storage_encrypted = 1`; a device without the key refuses to upload before touching the bucket. Turning it on encrypts nothing already uploaded; "Re-upload existing recordings encrypted" sends the plain objects' files up again, one at a time, resumable by the parts journal, and remembers the plain objects for the purge tag. Off makes new uploads plain and leaves encrypted objects readable by any device with the key. |
| ENC-4 | **Local copies are plain** on a device with the key: a download and a fetch open the object on arrival, written whole before the file exists. A device without the key keeps the bytes as the bucket holds them, marked by their header; it serves them as they are with `x-voice-encrypted: 1` and the hash of those bytes, and the device that fetches from it opens them with its key. A send between two devices with the key is plain over TLS. |
...
| PAIR-5 | **Grant**, for a server that holds no account and cannot reach the holder: `account host [--label]` on the server prints a grant text (`g=1`, no account id; its token is hashed in the root's `hosting_offers`, ten minutes, once). The holder runs `account grant-host <text>` (a phone pastes it into the same field as a code): it makes a key for the server, posts `/pair/grant` with the account id, that key, its own card and a label, and the server registers the account as hosted (never default), stores the key, admits the holder's card, writes its own card and answers with it. The holder admits the server's card and adds it as a peer. A grant text is refused by `join` and a code by `grant-host`, before any network. |
| AUTH-8 | **A request log per hosted account**, `audit.log` beside its database: one line per request with the time, device id, method, route, bytes in and out, status and refusal code. Never a key, never content. Rotated at 5 MiB with two old files kept (TECHNICAL-DECISIONS 7.1). A single-account listener writes none; its own log carries the lines. |
| AUTH-9 | **The phone keeps its device key wrapped** (Stage 14): `Config::new` takes an optional `SecretWrapper`, the Android Keystore through the `KeystoreWrapper` callback (`SecretWrap`, AES-256-GCM under a hardware key that never leaves the phone). On disk `config.json` holds `device_key_wrapped` and an empty `device_key`; the clear key is in memory only; a file with a clear key is wrapped at the first open under a wrapper. The desktop keeps `config.json` at mode 0600 without wrapping. The bucket secret is a synced setting and travels in clear inside the database; wrapping it would need a local shadow that the feed excludes, which is an open decision. |
| DIAG-1 | **A page is committed before the next is requested.** The client applies a page, saves the cursor, and only then asks for the next; a connection cut mid-sync leaves the cursor at the end of the last page applied and the next sync continues from there, asking only for the rest. |
| DIAG-2 | **A request id per operation.** Every button press (a sync, deliver, exchange, send, fetch, join, grant, check) begins an operation with a fresh 16-hex-character id, sent as `X-Request-ID` on every request of it. The server runs each request inside a tracing span carrying the id and the caller's device, writes it in the audit line, and the result carries it (`request_id`); the command line prints "Request <id>" after every result, the phone shows it under the result. |
| DIAG-3 | **Clock skew is reported.** The handshake's `server_timestamp` is compared with this device's clock; a difference past a minute is a warning on the result ("This device's clock is 4 minutes behind Desk's") and `clock_skew_seconds`. Below a minute, nothing. |
| DIAG-4 | **A connection check**, `sync check <peer>` and "Check connection" on the phone: one row each for reachability (a status request without a key), the certificate (pinned or by the roots; a certificate error is `CERTIFICATE_MISMATCH`), the account (`ACCOUNT_*`), the key (`DEVICE_*`, `KEY_*`), the clock, free space on both sides (the handshake now carries `free_bytes`), whether the peer serves recordings, and whether this device listens. Nothing is changed by a check. |
| DIAG-5 | **The changes feed is gzipped** when the caller sends `Accept-Encoding: gzip` (every client of the family does); the file routes never are. |
| PROOF-1 | **One line, in one place**: "3 notes and 2 recordings are not duplicated off this device." at the top of the phone's sync screen and in `sync status` (the desktop's sync dialogue when it exists). Never a notification, a badge or a reminder. A note counts as here only when its head content version was not received by sync, is authored or the root of a note row written here, and has a `seq` above every peer's sent cursor; a recording when its file is here, its row has no `storage_key`, and no peer is known to hold it. "Everything is duplicated off this device." when both are zero. |
| PROOF-2 | **Where the copies are**: the local table `audio_file_copies(audio_id, peer_id, at)` (never synced) is written by a send (the receiver holds it), by a receive (the sender holds it), by a fetch (the source holds it), and by a missing-list request (the caller holds every id it names); a fetcher posts its held list once more after fetching so the source learns what it now holds. A recording's details name this device, the bucket and each peer. |
| PROOF-3 | **Per peer**: `sync_peers.last_sync_at` and `last_operation` (sync, deliver, exchange, send, fetch) are written at the end of every operation with the peer; `sync status` and the sync screen list them. |
...
| SNAP-5 | **The periodic backup**, separate from the snapshots: every `backup.interval_hours` (default 24; 0 turns it off) each open account's database is quiesced (the write lock held, the write-ahead log checkpointed and truncated), copied with the backup API to `backup.directory` (default `<root>/backups/<account id>/`), named by time, keeping the newest `backup.keep` (default 30). It runs inside the listener (`sync serve`, the desktop's Listen for peers) for every open account, and inside the desktop while it is open, checked every quarter hour; `account backup` runs it now. The three values are in the machine configuration. Never inside the recordings folder; nothing copies it off the machine. |
...
| Endpoint | Purpose |
|----------|---------|
| `POST /sync/handshake` | Exchange `device_id`, `device_name`, `protocol_version` (`"2.0"`; a `1.x` caller is refused with `PROTOCOL_TOO_OLD`, "Update Voice on <device name>"), `application` (`"voice"`) and `entity_types` (the types the caller wants; empty means all); returns the server's identity, `database_id`, `cursor` (end of its feed), `supports_audiofiles`, and for older tools `last_sync_timestamp` and `server_timestamp`. |
...
| FILE-6 | The file on disk is named by the row's `disk_name` (FILE-15), never by the id. The bucket object is `<content hash>.<ext>` (FILE-18), `.enc` added when encrypted (ENC-3), and `<id>.<ext>` only while the row has no hash (`storage_key_for`). `ext` is from `audio_file_extension()`: lowercase, last dot wins, `bin` when absent. Every platform uses the shared helpers. |
...
| FILE-14 | Timeouts are explicit: three seconds to connect on this machine or a private address, ten elsewhere; thirty seconds for a read to make progress, on a sync's requests as on a file's; no overall timeout on a file. An upload (a send to a peer, a part or a small file to the bucket) has no read timeout, because the connection reads nothing while a body goes out: it ends when no byte of its body has moved for thirty seconds, or one minute after the last byte without an answer (`transfer::stall_of_upload`). A download from the bucket is read as it arrives and ends when a read moves nothing for thirty seconds; the bucket's existence check has thirty seconds. A transfer is tried three times with waits of one, two and four seconds; a try after a broken one asks the peer how many bytes it holds and continues from there; a refusal (4xx) is not retried; the metadata routes do not retry. A transfer that breaks keeps the bytes that arrived in its part file on both sides. The receiver refuses a file that would leave less than 64 MB free, naming both numbers. |
...
| UI-9 | **One visible button, naming the last peer** (Stage 5): the desktop's sync dialogue (File → Sync…) and the phone's sync screen show "Exchange with Desk", where Desk is the peer of the last operation (`last_peer_id`) or the only peer; the arrow beside it chooses another peer or another of the five operations. With no peer yet the button says so and points at the code. Every result is one sentence naming the operation and the peer, ending with the request id; a refusal is one sentence, its code, and the button that fixes it (Show my code, Check connection). |
| UI-10 | **The sync dialogue** holds, top to bottom: the proof line (PROOF-1), the peers with when each was last reached and by what, the one button, Check connection, Show my code (the QR code and the text, the listener started for it), Rename, Forget, Add by address, the result, the Listen for peers switch, and this device's name, id, address, port and certificate. Every control has an accessible name or description; no state is shown by colour alone. The "changes pending" mark on the Sync menu item and its timer are gone: the line is the only place. |
| UI-11 | **A device name that means something**: the desktop's default is the hostname alone; the phone's is its model as Android reports it, set on the first start in place of the core's placeholder. |
| UI-12 | **The phone shows and reads the code** (Stage 9). "Show my code" starts the listener, makes a setup text (PAIR-1), draws it as a QR code beside "treat this like a password", with Copy and Share, and hides it after sixty seconds. "Read a code" opens the camera with the paste field under it; every camera is tried in turn, rear first and front last, and the next is tried when one fails to open or gives no frame for three seconds; a code that is not a setup text is ignored. The setup text is a `voice://pair` link the phone registers, so one tapped in a messaging app opens the sync screen and uses it. The first run with no notes offers "Pair with another device" (the reader) and "Start on my own". After pairing, the new peer is shown with one Exchange (or, for a grant, Deliver) button and Later. |
...
| ACCT-6..9 | `accounts.rs` tests (a first run makes the index and the default once; a directory holding a database is the account; id prefix and label; unique labels; a hosted account never default; a disagreeing database reported; machine settings shared); `tests/cli/test_cli_accounts_index.py` |
...
| FILE-15 | `database.rs::tests::an_imported_file_keeps_its_name_and_a_taken_name_gets_a_numbered_suffix`, `database.rs::tests::a_row_from_before_the_name_column_keeps_the_name_its_file_has`, `models.rs` (`a_recording_is_named_by_its_start_at_its_own_offset_and_the_tail_of_its_id`, `a_taken_name_gets_a_number_before_its_extension`), `sync_server.rs::tests::files_between_instances` (every path through the row), `tests/unit/test_audiofile_manager.py` |
| BUCKET-1..5 | `bucket_setup.rs` tests (the policy, the cleaned key, the explained failures, the documented signature example, the three rules); `tests/sync/test_storage_setup.py` against `tests/fake_s3.py` (a small S3 in a thread that refuses every delete) |
| LISTEN-3 | `sync_server.rs::tests::lan_only` |
| DISC-1..2 | `tests/sync/test_sync_discovery.py` (the hash, real mDNS on the loopback, the remembered address first and the browse only on silence) |
| CARD-3, UI-9..11 | `sync_server.rs::tests::peers_from_cards`, `config.rs` tests (the forgotten peer, the hostname); `tests/gui/test_sync_dialog.py`; `tests/cli/test_cli_peers.py` |
| UI-12 | `app/src/test/.../ui/components/QrReaderTest.kt` (a code drawn and read back from a frame, a code for something else ignored, the camera order and the round, no camera at all, what is a setup link); the camera and the screen by hand |
| FILE-17 | `sync_server.rs::tests::files_between_instances::progress_is_reported_and_a_cancel_stops_at_the_next_chunk_leaving_a_part_to_continue_from` |
| FILE-18 | `database.rs::tests::the_content_hash_is_stored_from_the_file_and_travels_by_sync_and_is_never_erased`, `file_storage.rs::tests::a_key_is_by_the_content_hash_when_the_row_has_one`, `file_storage.rs::tests::batch::a_download_is_verified_against_the_rows_content_hash`, `sync_server.rs::tests::files_between_instances::a_send_stores_the_hash_once_and_the_peer_learns_it_by_sync` |
| FILE-19 | `file_storage.rs::tests::parts` (the loop against a store in memory: the failed part alone sent again, the cancel, a changed part size, the database journal); `tests/sync/test_storage_setup.py::test_a_large_recording_goes_up_in_parts_and_a_dropped_part_is_the_only_one_sent_again` (the three signed requests against the fake bucket) |
| FILE-20 | `waveform.rs::tests` (`bars_are_the_loudest_level_of_each_share_scaled_to_the_loudest`, `levels_round_trip_through_text_and_too_many_are_refused`), `database.rs::tests::waveform_levels_are_kept_synced_and_drawn_without_the_audio`, `tests/unit/test_waveform_accumulator.py::test_the_levels_kept_with_a_recording_draw_the_same_bars`, `tests/cli/test_cli_audiofiles.py::TestAudiofilesWaveforms`, Android `WaveformAccumulatorTest` (`the levels kept with a recording draw the bars this phone draws`) |
| FILE-21 | `models.rs::tests` (the common formats are on the list), `tests/unit/test_audiofile_edge_cases.py::test_every_common_audio_format_is_supported` |
| FILE-14 | `tests/sync/test_sync_over_a_failing_network.py` (refused, 503, a push refused or its reply lost, a pull cut or frozen, a dead link, a slow link, repeated drops; a send or a fetch cut, frozen or slower than the read timeout), `tests/integration/test_file_storage.py::TestTheBucketOverAFailingNetwork` (refused, 503 on a part, a part cut, a frozen upload or download, a bucket that never answers, a part slower than the stall timeout), all through `tests/faulty_network.py` against moto's S3 (`tests/local_s3.py`) and the listener |
| FILE-22 | `database.rs::tests::file_locations` (statements, either order, the removal while another device uploads, the folder, the only copy, purge, an older database), `convergence_tests.rs` (location statements in the randomized fleets), `sync_server.rs::tests::files_between_instances::where_each_copy_is_is_known_on_both_devices`, `tests/sync/test_file_locations_sync.py`, `tests/integration/test_file_storage.py::TestWhereTheCopiesAre`, `tests/unit/test_file_locations.py`, `tests/cli/test_cli_issues.py`, `tests/web/test_api_issues.py`, Android `CoreLocationsTest` |
| FILE-23 | `database.rs::tests::file_locations::the_upload_limit_is_the_accounts_and_travels_with_the_storage_configuration`, `file_storage.rs::tests::batch::a_file_over_the_accounts_upload_limit_stays_where_it_is`, `tests/cli/test_cli_issues.py::test_the_upload_limit_is_shown_and_set_once_a_bucket_exists`, Android `CoreLocationsTest` |
| ISSUE-1 | `issues.rs::tests`, `tests/unit/test_issues_text.py`, `tests/gui/test_issues_dialog.py`, `tests/tui/test_tui_issues.py`, `tests/cli/test_cli_issues.py`, `tests/web/test_api_issues.py`, Android `IssuesTextTest` and `CoreLocationsTest` |
| ENC-1 | `config.rs::tests::the_device_key_is_written_wrapped_and_read_back_through_the_wrapper` (the recording key wrapped too), `sync_server.rs::tests::pairing::the_recording_key_travels_to_a_device_let_in_by_the_code`, `crypto.rs::tests::the_key_text_is_43_characters_and_round_trips`; `tests/sync/test_storage_setup.py::test_encrypted_uploads_are_enc_objects_that_open_only_with_the_key` (the switch refuses before an export); `tests/gui/test_sync_dialog.py::test_the_encryption_switch_waits_for_the_export_and_follows_the_synced_setting` |
| ENC-2 | `crypto.rs::tests` (every chunk boundary, the view at any offset, the wrong key, a change, a swap, a cut, a fresh nonce per file, written whole or not at all) |
| ENC-3 | `file_storage.rs::tests::batch::an_encrypted_upload_makes_an_enc_object_that_a_download_with_the_key_opens`, `reupload_encrypted_replaces_plain_objects_and_remembers_them_for_the_purge`; the desktop test above through the fake bucket |
| ENC-4 | the two tests above (a keyless download keeps the bytes, the wrong key refuses), `sync_server.rs::tests::files_between_instances::an_encrypted_file_served_by_a_keyless_device_is_opened_by_one_with_the_key` |
| SNAP-5 | `sync_server.rs::tests::periodic_backup`; `tests/cli/test_cli_account.py::test_backup_now_copies_the_database_and_keeps_the_newest` |
| ACCT-10 | `sync_server.rs::tests::pairing::a_device_with_notes_moves_to_another_account_by_its_code_and_tags_with_one_path_become_one`; `tests/sync/test_sync_pairing.py::TestMoveByCode` |
| PROTO-12..13 | `sync_server.rs::tests::version_two`; `tests/sync/test_sync_protocol.py::test_a_handshake_of_version_one_is_refused_in_words` |
| PROOF-1..3 | `sync_server.rs::tests::files_between_instances` (`what_is_on_this_device_only_is_counted_until_it_is_elsewhere`, the copies both sides know after an exchange); `tests/sync/test_sync_proof.py` |
| DIAG-1..5 | `sync_server.rs::tests::robustness` (a sync cut after its first page, the feed gzipped or plain, the connection check's rows), `sync_client.rs` unit tests (the skew sentence, the id's shape), the request id in the audit log in `sync_server.rs::tests::hosting`; `tests/sync/test_sync_diagnostics.py` |
| AUTH-4, AUTH-8, PAIR-5 | `sync_server.rs::tests::hosting` (a grant, a spent grant, a claim through the host, the audit log, a stranger's account, a single listener taking no grant); `tests/sync/test_sync_hosting.py` (the same from the command line, a directory that cannot host, a grant text and a code refused by the other command) |
| AUTH-9 | `config.rs::tests::the_device_key_is_written_wrapped_and_read_back_through_the_wrapper`; `app/src/test/.../util/SecretWrapTest.kt` (round trip, a fresh nonce, another key or a changed byte refused) |
