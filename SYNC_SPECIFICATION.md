...
| FILE-15 | **A recording's file is named by its row** (`audio_files.local_name`, local, never synced): `YYYY_MM_DD_HH_MM_SS-<last eight of the id>.<ext>`, the start rendered at the recording's own offset; written once at import, at recording, or on arrival by sync, and never renamed. Every lookup on every platform goes through it; nothing derives a path from the id and the original name. |
...
| FILE-15 | `models.rs` (`a_recording_is_named_by_its_start_at_its_own_offset_and_the_tail_of_its_id`), `sync_server.rs::tests::files_between_instances` (every path through the row), `tests/unit/test_audiofile_manager.py` |
