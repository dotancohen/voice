...
| FILE-15 | **A recording's file name is on its row** (`audio_files.local_name`, local, never synced) and is always the name the file has on disk. A file this application records is named `YYYY_MM_DD_HH_MM_SS-<last eight of the id>.<ext>`, the start rendered at the recording's own offset. An imported file keeps its own name, any POSIX name, in the same folder. A name already taken in the folder, by a row or by a file, gets ` (2)`, ` (3)` and so on before its extension; names are compared without regard to case, as the phone's shared storage compares them. The copy into the folder never overwrites a file. A row arriving by sync is named like a recording. A row from before the column keeps the name its file has, `<id>.<ext>`: no migration changes a name that refers to a file. Every lookup on every platform goes through the row's name. |
