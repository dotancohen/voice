...
`FaultyLink` between a device and its peer and checks three things: the
...
def linked(tmp_path: Path) -> Generator[Tuple[SyncNode, SyncNode, FaultyLink], None, None]:
    ...
    node_a.config.add_peer(node_b.device_id_hex, node_b.name, link.url)
...
class TestSyncOverAFailingLink:
    def test_a_refused_connection_fails_at_once_and_the_next_sync_completes(self, linked) -> None:
        ...
        link.refuse()
        result, took = within(30, lambda: client_of(node_a).sync_with_peer(node_b.device_id_hex))
    ...
    def test_an_error_status_on_the_way_fails_the_sync_and_the_next_one_completes(self, linked) -> None:
        ...
        result = client_of(node_a).sync_with_peer(node_b.device_id_hex)
    ...
    def test_a_push_refused_with_500_is_sent_again_by_the_next_sync(self, linked) -> None:
        ...
        result = client_of(node_a).sync_with_peer(node_b.device_id_hex)
        assert result.success is False, "a push the peer did not take is not a success"
        assert contents(node_b, ids) == {}

        link.pass_through()
        again = client_of(node_a).sync_with_peer(node_b.device_id_hex)
        assert again.success, again.errors
        assert contents(node_b, ids) == contents(node_a, ids)

    def test_a_push_whose_reply_is_lost_is_applied_once(self, linked) -> None:
        """The peer applied the push, and the reply saying so never came back.
        The next sync sends the same changes again; the peer must take them as
        ...
        result = client_of(node_a).sync_with_peer(node_b.device_id_hex)
        assert result.success is False
        assert contents(node_b, ids) == contents(node_a, ids), "the peer applied the push before its reply was lost"
    ...
    def test_a_dead_link_ends_the_sync_in_bounded_time(self, linked) -> None:
        ...
        notes_on(node_a, 2)
        link.stall()
        result, took = within(150, lambda: client_of(node_a).sync_with_peer(node_b.device_id_hex))
        assert result.success is False and result.errors
    ...
    def test_a_slow_link_completes(self, linked) -> None:
        ...
        result, _ = within(120, lambda: client_of(node_a).sync_with_peer(node_b.device_id_hex))
    ...
    def test_edits_on_both_sides_through_repeated_drops_converge(self, linked) -> None:
        ...
        for round_ in range(4):
            ...
            client_of(node_a).sync_with_peer(node_b.device_id_hex)
        link.pass_through()
        for _ in range(2):
            result = client_of(node_a).sync_with_peer(node_b.device_id_hex)
            assert result.success, result.errors
...
class TestRecordingsOverAFailingLink:
    def test_a_send_cut_in_the_middle_is_resumed_and_arrives_whole(self, linked) -> None:
        ...
        for _ in range(4):
            result = client_of(node_a).send_to_peer(node_b.device_id_hex)
            node_b.reload_db()
    ...
    def test_a_fetch_cut_in_the_middle_is_resumed_and_arrives_whole(self, linked) -> None:
        ...
        for _ in range(4):
            client_of(node_a).fetch_from_peer(node_b.device_id_hex)
    ...
    def test_a_send_over_a_link_slower_than_the_read_timeout_completes(self, linked) -> None:
        """A phone's uplink can take minutes over one recording while the peer
        ...
        result, took = within(240, lambda: client_of(node_a).send_to_peer(node_b.device_id_hex))
    ...
    def test_a_send_over_a_link_that_freezes_ends_in_bounded_time_and_leaves_no_file(self, linked) -> None:
        ...
        link.pass_through()
        assert client_of(node_a).send_to_peer(node_b.device_id_hex).sent == 1
        assert local_path(node_b, audio_id).read_bytes() == path.read_bytes()
    ...
    def test_a_fetch_over_a_link_that_freezes_ends_in_bounded_time_and_leaves_no_file(self, linked) -> None:
        ...
        link.pass_through()
        assert client_of(node_a).fetch_from_peer(node_b.device_id_hex).fetched == 1
        assert local_path(node_a, audio_id).read_bytes() == path.read_bytes()
